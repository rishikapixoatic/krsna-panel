...
const WeddingPage = () => {
  ...
  const [thumbnailStep2, setThumbnailStep2] = useState(null);
  ...
  const [errorMessageOfStep2, setErrorMessageStep2] = useState('');
  const [errorMessageOfStep3, setErrorMessageStep3] = useState('');
  const [successMessageofStep4, setSuccessMessageStep3] = useState('');
  const [changeAtBack, setChangeAtBack] = useState(false);
  const [showBackButton, setShowBackButton] = useState(false);
  const [subEventFolder, setSubEventFolder] = useState();
  const [successMessageofStep2, setSuccessMessageStep2] = useState('');
  const [changeAtBackStep2, setChangeAtBackStep2] = useState(false);
  ...
  useEffect(() => {
    validateFormStep2();
    changeAtBackStep2 && setChangeAtBackStep2(false);
  }, [thumbnailStep2, subEventFolder]);

  const validateFormStep2 = () => {
    setFormIsValidStep2(thumbnailStep2 && subEventFolder)
  }
  ...
  const handleSubmitStep2 = async (e) => {
    ...
    formData.append('thumbnail', thumbnailStep2);
    console.log("handleSubmitStep2");
    try {
      if (currentStep === 2) {
        if (!changeAtBackStep2) {
          const response = await createFolder(formData, accessToken);
          if (response.success) {
            setSuccessMessageStep2('Folder created successfully.');
            setCurrentStep(3);
            setShowBackButton(true);
            setErrorMessageStep2('');
          } else {
            setErrorMessageStep2(response.error.message);
            setFormIsValidStep2(false);
            setSuccessMessageStep2('');
          }
        }
        else {
          setCurrentStep(3);
      ...
      setErrorMessageStep2('Failed to create folder. Please try again.');
    }
  }
  ...
  return (
    <div className="flex h-full w-full bg-gray-100">
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <h2 className="text-lg font-semibold m-5">Wedding</h2>
          <div className="min-h-full pt-5 px-4 sm:px-6 lg:px-8">
            <div className='flex'>
              <WeddingList accessToken={accessToken}/>
              <div className="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden">
                <div className="p-6">
                  ...
                  {currentStep === 2 && (
                    <>
                      <h2 className="text-xl font-semibold mb-4">Folder Selection</h2>
                      <form className="mt-8 space-y-6" onSubmit={handleSubmitStep2}>
                        <div className="rounded-md -space-y-px">
                          <div className="flex flex-col gap-y-4 items-center">
                            {!thumbnailStep2 ? <div>
                              <label htmlFor="thumbnailStep2" className="sr-only">Thumbnail</label>
                              <input id="thumbnailStep2" name="thumbnailStep2" type="file" accept="image/*" aria-describedby="validFileFormats" required onChange={(e) => setThumbnailStep2(e.target.files[0])} />
                            </div> : (
                              <div className="flex items-center space-x-2">
                                <p>{thumbnail.name}</p>
                                <button
                                  type="button"
                                  className="text-sm text-gray-900 bg-[#DDE4F7] w-16 h-8"
                                  onClick={() => setThumbnailStep2(null)}
                                >
                                  Change
                                </button>
                              </div>
                        ...
                        {errorMessageOfStep2 ? <ErrorMessage message={errorMessageOfStep2} /> : <p className="mt-2 text-sm text-green-600">{successMessageofStep2}</p>}
                        <div className="flex justify-between">
                          <button className="text-gray-500 hover:text-gray-800" onClick={handleBack}>
                            Back
                          </button>
                          <button type="submit" disabled={!formIsValidStep2} className={`group relative flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white ${!formIsValidStep2 ? 'bg-gray-500' : 'bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500'}`}>
                            Next
                          </button>
                        </div>
                      </form>
                  ...
                  {currentStep === 3 && (
                    <>
                      <h2 className="text-xl font-semibold mb-4">Upload Images and Videos to {selectedFolder}</h2>
                      <form className="mt-5 space-y-6" onSubmit={handleSubmitStep3}>
                        <input
                          id="files"
                          name="files"
                          type="file"
                        ...
                        {errorMessageOfStep3 ? <ErrorMessage message={errorMessageOfStep3} /> : <p className="mt-2 text-sm text-green-600">{successMessageofStep4}</p>}
                        <div className="flex justify-between">
                          <button className="text-gray-500 hover:text-gray-800" onClick={handleBack}>
                            Back
                          </button>
